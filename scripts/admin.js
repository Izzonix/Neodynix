...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchChatRequests() {
    try {
      ...
      messages.forEach(request => {
        const isFile = request.content.includes('📎 Sent file:');
        const fileUrl = request.file_url || '';
        const card = document.createElement('div');
        card.className = 'chat-request-card';
        card.innerHTML = `
          <h3>${request.users.name}</h3>
          <p>Email: ${request.users.email}</p>
          <p>Topic: ${request.users.topic || 'N/A'}</p>
          <p>Message: ${request.content}</p>
          ${isFile ? `
            <p>File: <a href="${fileUrl}" target="_blank" download="${fileUrl.split('/').pop()}">Download</a></p>
            <div class="button-container">
              <button class="btn" onclick="replyToChat('${request.id}', '${request.users.name}', '${request.users.email}')">Reply</button>
              <button class="btn btn-delete" onclick="deleteChatMessage('${request.id}', '${fileUrl}')">Delete</button>
            </div>
          ` : `
            <div class="button-container">
              <button class="btn" onclick="replyToChat('${request.id}', '${request.users.name}', '${request.users.email}')">Reply</button>
              <button class="btn btn-delete" onclick="deleteChatMessage('${request.id}')">Delete</button>
            </div>
          `}
        `;
  ...
  window.deleteChatMessage = (id, fileUrl = '') => {
    showConfirm('Are you sure you want to delete this message?', async () => {
      ...
      try {
        ...
        chatResult.textContent = 'Message deleted successfully!';
        fetchChatRequests();
      } catch (error) {
        console.error('Error deleting message:', error);
        chatResult.className = 'result error';
        chatResult.textContent = 'Failed to delete message: ' + error.message;
      } finally {
        loadingPopup.style.display = 'none';
      }
    });
