  ...
  async function fetchChatRequests() {
    try {
      const { data: messages, error: msgError } = await supabase
        .from('messages')
        .select('id, email, message, created_at, replied, users(name)')
      ...
      messages.forEach(msg => {
        ...
        if (!users.has(email)) {
          users.set(email, { name: msg.users?.name || 'Unknown User', messages: [] });
        }
        users.get(email).messages.push({
          id: msg.id,
          message: msg.message,
        ...
        user.messages.forEach(msg => {
          messagesHtml += `
            <li>
              ...
              <button class="btn" onclick="replyToChat('${msg.id}', '${user.name}', '${email}')" ${msg.replied ? 'disabled' : ''}>Reply</button>
            </li>
          `;
        });
        messagesHtml += '</ul>';
