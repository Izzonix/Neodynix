  ...
  function initAdmin() {
    ...
    window.editTemplate = async (id) => {
      ...
      try {
        ...
        if (fetchError) throw new Error('Failed to fetch template.');
        ...
        imagePreview.innerHTML = existingData.image ? `<img src="${existingData.image}" alt="preview" style="max-width: 100%; border-radius: 6px;">` : '';
        ...
        imageFile.addEventListener('change', () => {
          ...
          if (file) {
            const reader = new FileReader();
            reader.onload = (e) => {
              imagePreview.innerHTML = `<img src="${e.target.result}" alt="preview" style="max-width: 100%; border-radius: 6px;">`;
            };
            reader.readAsDataURL(file);
          } else {
            imagePreview.innerHTML = existingData.image ? `<img src="${existingData.image}" alt="preview" style="max-width: 100%; border-radius: 6px;">` : '';
