    ...
    try {
      ...
      data.forEach(msg => {
        if (!users.has(msg.email)) {
          users.set(msg.email, { name: msg.name, messages: [] });
        }
        users.get(msg.email).messages.push(msg);
      });
      users.forEach(user => {
        user.messages.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
        user.messages = user.messages.slice(0, 5);
        user.name = user.messages[0].name;
      });
      users.forEach((user, email) => {
        const card = document.createElement('div');
        card.className = 'chat-request-card';
        let messagesHtml = '<ul>';
        user.messages.forEach(request => {
          messagesHtml += `
            <li>
              <p>Message: ${request.message}</p>
              <p>Sent: ${new Date(request.created_at).toLocaleString()}</p>
              <p>Status: ${request.replied ? 'Replied' : 'Pending'}</p>
              <button class="btn" onclick="replyToChat('${request.id}', '${user.name}', '${email}')" ${request.replied ? 'disabled' : ''}>Reply</button>
            </li>
          `;
        });
        messagesHtml += '</ul>';
